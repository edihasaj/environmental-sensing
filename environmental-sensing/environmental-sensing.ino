...
unsigned int sendEvery = 600;                           // Creates a delay so the data is not constantly sent. 
uint8_t sensorType;                                     // Determines tph sensor type
bool sendFirstTime = true;
bool sendFirstLoadOfData = true;
...
void sendSensorValues2() {                               // Function used to send the data we collected from all the sensors
  payload.reset();                                      // Reset the cbor payload (in case there's anything left from the previous one)
...
//  delay(sendEvery*1000);                                // Creates a delay so the data is not constantly sent. The delay is defined in the "sendEvery" variable at the beginning.
  ...
  if (sendFirstLoadOfData) {
    sendSensorValues();                                   // Calls the function that sends all the sensor data to AllThingsTalk Maker via LoRa
    sendFirstLoadOfData = false;
  } else {
    sendSensorValues2();                                   // Calls the function that sends all the sensor data to AllThingsTalk Maker via LoRa
    sendFirstLoadOfData = true;
  }
